window.t_forms__lang = (window.navigator.userLanguage || window.navigator.language).toUpperCase().slice(0, 2), window.scriptSysPayment = {}, window.handlerSysPayment = {}, window.isInitEventsZB = {}, window.isInitEventsCustomMask = {}, window.initForms = {}, window.tildaForm = {
    ...
    if (allrecords) {
        ...
        rootzoneapi && (window.tildaForm.endpoint = "forms.tildaapi." + rootzoneapi)
    }
    t_forms__initForms();
    var isST100 = !!document.querySelector(".t706"), isST105 = !!document.querySelector(".js-payment-systembox"),
        isExistsPay = !!document.querySelector("input[name=tld_product]");
    if (isST100 || isST105 || isExistsPay) {
        var fileName = "tilda-forms-payments-1.0", script;
        if (!document.head.querySelector('script[src*="' + fileName + '"]')) (script = document.createElement("script")).type = "text/javascript", script.src = "/js/" + fileName + ".min.js", script.onerror = function () {
            console.error("Failed to load tilda-forms-payments: ", this.src)
        }, document.head.appendChild(script)
    }
    var lang = window.t_forms__lang;
    if ("RU" !== lang && "EN" !== lang) {
        var fileName = "tilda-forms-dict-1.0", script;
        if (!document.head.querySelector('script[src*="' + fileName + '"]')) (script = document.createElement("script")).type = "text/javascript", script.src = "/js/" + fileName + ".min.js", script.onerror = function () {
    ...
    if (!0 !== window.isInitEventsCustomMask) {
        ...
        script.type = "text/javascript", script.src = "/js/tilda-forms-custommask-1.0.min.js", document.head.appendChild(script), script.onload = function () {
    ...
    if (inputsCustomMask.length && !0 !== window.isInitEventsCustomMask) return window.tildaForm_customMasksLoad(), void window.setTimeout((function () {
    ...
    for (var inputs = form.querySelectorAll(".js-tilda-rule"), arrError = [], isEmptyValue = !0, i = 0; i < inputs.length; i++) {
        var input = inputs[i], inputGroup = input.closest(".t-input-group"), isHidden;
        if (!(inputGroup && "true" === inputGroup.getAttribute("data-hidden-by-condition"))) {
            var isReq = !!parseInt(input.getAttribute("data-tilda-req") || 0, 10),
                dataRule = input.getAttribute("data-tilda-rule") || "none", regExp = "", strValue = "",
                minLength = input.getAttribute("data-tilda-rule-minlength") || 0,
                maxLength = input.getAttribute("data-tilda-rule-maxlength") || 0, objError = {}, value = input.value,
                valueNoSpace = "", inputType = input.getAttribute("type"), inputName = input.getAttribute("name"),
                dataFormCart = form.getAttribute("data-formcart");
            objError.obj = input, objError.type = [], value && value.length && (valueNoSpace = value.replace(/[\s\u0000—\u001F\u2000-\u200F\uFEFF\u2028-\u202F\u205F-\u206F]/gi, ""), value = value.trim()), value.length > 0 && (isEmptyValue = !1), minLength && (minLength = parseInt(minLength, 10)), maxLength && (maxLength = parseInt(maxLength, 10));
            ...
            if (isReq && (isEmpty || isCheckBoxOrRadio && isChecked)) objError.type.push("req"); else {
                switch (dataRule) {
                    case"email":
                        regExp = /^(?!\.)(?!.*\.\.)[a-zA-Zёа-яЁА-Я0-9\u2E80-\u2FD5\u3190-\u319f\u3400-\u4DBF\u4E00-\u9FCC\uF900-\uFAAD_\.\-\+]{0,63}[a-zA-Zёа-яЁА-Я0-9\u2E80-\u2FD5\u3190-\u319f\u3400-\u4DBF\u4E00-\u9FCC\uF900-\uFAAD_\-\+]@[a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéû0-9][a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéû0-9\.\-]{0,253}\.[a-zA-Zёа-яЁА-Я]{2,11}$/gi, value.length && !value.match(regExp) && objError.type.push("email");
                        break;
                    case"url":
                        regExp = /^((https?|ftp):\/\/)?[a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéûşç0-9][a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéûşç0-9_\.\-]{0,253}\.[a-zA-Zёа-яЁА-Я]{2,10}\/?$/gi, value.length && ((strValue = (strValue = (strValue = value.split("//")) && strValue.length > 1 ? strValue[1] : strValue[0]).split("/")) && strValue.length && strValue[0] ? (strValue = strValue[0]).match(regExp) || objError.type.push("url") : (strValue && !strValue[0] || objError.type.push("url"), strValue = ""));
                        break;
                    case"phone":
                        var phoneMask = input.getAttribute("data-tilda-mask"), strRegExp = "^[0-9()+-";
                        ...
                        break;
                    case"number":
                        regExp = /^[0-9]+$/gi, valueNoSpace.length > 0 && !valueNoSpace.match(regExp) && objError.type.push("number");
                        break;
                    case"date":
                        var format = {
                        ...
                        break;
                    case"time":
                        regExp = /^[0-9]{2}[:\.][0-9]{2}$/gi, valueNoSpace.length > 0 && !valueNoSpace.match(regExp) && objError.type.push("time");
                        break;
                    case"name":
                ...
                minLength > 0 && value.length && value.length < minLength && objError.type.push("minlength"), maxLength > 0 && value.length && value.length > maxLength && objError.type.push("maxlength")
            }
            objError.type && objError.type.length && (arrError[arrError.length] = objError)
        }
    }
    if ("y" === dataFormCart) {
        var isMinOrderSetted = window.tcart_minorder > 0, isMinQuantitySetted = window.tcart_mincntorder > 0,
            prodAmount, objError, objError;
        if (isMinOrderSetted) if ((window.tcart.prodamount_withdiscount > 0 ? window.tcart.prodamount_withdiscount : void 0 !== window.tcart.prodamount_withdyndiscount && window.t_cart__discounts && window.t_cart__discounts.length > 0 ? window.tcart.prodamount_withdyndiscount : window.tcart.prodamount) < window.tcart_minorder) (objError = {
            obj: {},
            type: []
        }).type.push("minorder"), arrError.push(objError);
        if (isMinQuantitySetted && window.tcart.total < window.tcart_mincntorder) (objError = {
            obj: {},
            type: []
        }).type.push("minquantity"), arrError.push(objError)
    }
    return isEmptyValue && !arrError.length && (arrError = [{obj: "none", type: ["emptyfill"]}]), arrError
}, window.tildaForm.hideErrors = function (form) {
    if ("object" != typeof form || form.length) {
        form instanceof Element || (form = form[0]);
        var errorBoxes = form.querySelectorAll(".js-errorbox-all"), errorRule = form.querySelectorAll(".js-rule-error"),
        ...
        })), t_removeClass(form, "js-send-form-error"), t_removeClass(form, "js-send-form-success"), errorPopup && t_fadeOut(errorPopup)
    }
    ...
    var formId = form.getAttribute("id"), inputBoxClassName = form.getAttribute("data-inputbox");
    inputBoxClassName || (inputBoxClassName = ".blockinput");
    var inputGroup = "", isErrorBox = !1, isShowErrors = !0, errorItem = "", errorInputs = "", strError = "",
        strCommonError = "", popupError = document.getElementById("tilda-popup-for-error");
    ...
    for (var i = 0; i < arrErrors.length; i++) if (arrErrors[i] && arrErrors[i].obj) {
    ...
    if (strCommonError) {
        ...
        if (Array.prototype.forEach.call(errorsText, (function (text) {
            text.style.display = "block"
        })), document.querySelector('.t1093 [data-elem-type="form"]')) {
            var activeZeroPopupHookList = window.tPopupObj && window.tPopupObj.openPopUpList;
            if (activeZeroPopupHookList && activeZeroPopupHookList.length) {
                var activeZeroPopupHook,
                    activePopupSelector = '.t1093 .t-popup[data-tooltip-hook="' + activeZeroPopupHookList[activeZeroPopupHookList.length - 1] + '"]',
                    activeZeroPopup = document.querySelector(activePopupSelector),
                    currentZIndex = activeZeroPopup ? getComputedStyle(activeZeroPopup).zIndex : 0,
    ...
    function t_forms__hidePopup(event) {
        ...
        if ("INPUT" === ((event = event || window.event).target || event.srcElement).tagName) {
            ...
            t_fadeOut(popupError), Array.prototype.forEach.call(errorInputs, (function (input) {
                input.innerHTML = "", t_fadeOut(input)
            })), window.t_forms__errorTimerID && (window.clearTimeout(window.t_forms__errorTimerID), window.t_forms__errorTimerID = 0), window.isInitEventsZB[formId] = !0
        }
    }
    ...
    if (window.t_forms__errorTimerID && window.clearTimeout(window.t_forms__errorTimerID), window.t_forms__errorTimerID = window.setTimeout((function () {
        t_fadeOut(popupError), errorInputs = form.querySelectorAll(".t-input-error"), Array.prototype.forEach.call(errorInputs, (function (input) {
            input.innerHTML = "", t_fadeOut(input)
        })), window.t_forms__errorTimerID = 0
    }), 1e4), !window.isInitEventsZB[formId]) {
        var rec = form.closest(".r"), eventFocus = "focus";
        document.addEventListener || (eventFocus = "focusin"), t_removeEventListener(rec, eventFocus, t_forms__hidePopup), t_addEventListener(rec, eventFocus, t_forms__hidePopup, !0), t_removeEventListener(rec, "change", t_forms__hidePopup), t_addEventListener(rec, "change", t_forms__hidePopup, !0)
    }
    ...
    inputBoxClassName || (inputBoxClassName = ".blockinput");
    for (var inputGroup = "", isErrorBox = !1, isShowErrors = !0, errorItem = "", errorInputs = "", strError = "", inputErrors = [], i = 0; i < arrErrors.length; i++) if (arrErrors[i] && arrErrors[i].obj) {
        if (0 === i && "none" === arrErrors[i].obj) {
            errorItem = form.querySelectorAll(".js-rule-error-all");
            for (var j = 0; j < errorItem.length; j++) {
                errorItem[j].innerHTML = '<a href="#" class="t-form__errorbox-link">' + t_forms__getMsg("emptyfill") + "</a>", errorItem[j].style.display = "block";
                var firstInput = form.querySelectorAll(".t-input")[0], firstInputId;
                if (firstInput) t_forms__moveToErrorInput(firstInput.getAttribute("id"), errorItem[0])
            }
            break
        }
        var el = arrErrors[i].obj;
        el instanceof Element || (el = el[0]), el && (inputGroup = el.closest(inputBoxClassName)), inputGroup && (errorInputs = inputGroup.querySelectorAll(".t-input-error"), t_addClass(inputGroup, "js-error-control-box"), errorInputs.length > 0 && (isErrorBox = !0));
        for (var j = 0; j < arrErrors[i].type.length; j++) {
            var error = arrErrors[i].type[j], inputId = arrErrors[i].obj.id;
            if (strError = "", (errorItem = form.querySelectorAll(".js-rule-error-" + error)).length) for (var k = 0; k < errorItem.length; k++) {
                var currentErrorItem;
                (currentErrorItem = errorItem[k]).getAttribute("data-rule-filled") ? currentErrorItem.style.display = "block" : (currentErrorItem.textContent && currentErrorItem.innerText || !t_forms__getMsg(error) ? strError = errorItem[0].textContent || errorItem[0].innerText : currentErrorItem.innerHTML = '<a href="#" class="t-form__errorbox-link">' + t_forms__getMsg(error) + "</a>", currentErrorItem.style.display = "block", -1 === inputErrors.indexOf(error) && t_forms__moveToErrorInput(inputId, currentErrorItem), inputErrors.push(error))
            } else if (t_forms__getMsg(error) && (errorItem = form.querySelectorAll(".js-rule-error-all")).length) for (var k = 0; k < errorItem.length; k++) {
                var currentErrorItem;
                (currentErrorItem = errorItem[k]).innerHTML = '<a href="#" class="t-form__errorbox-link">' + t_forms__getMsg(error) + "</a>", currentErrorItem.style.display = "block", -1 === inputErrors.indexOf(error) && t_forms__moveToErrorInput(inputId, currentErrorItem), inputErrors.push(error)
            }
    ...
    var formCaptchaBox = document.getElementById("tildaformcaptchabox"),
        tildaCaptcha = document.getElementById("js-tildaspec-captcha"), randomKey;
    ...
    var strCaptcha = '<div id="tildaformcaptchabox" tabindex="-1" style="z-index: 99999999999; position:fixed; text-align: center; vertical-align: middle; top: 0px; left:0px; bottom: 0px; right: 0px; background: rgba(255,255,255,0.97);"><iframe id="captchaIframeBox" src="//' + window.tildaForm.endpoint + "/procces/captcha/?tildaspec-formid=" + form.getAttribute("id") + "&tildaspec-formskey=" + formKey + "&" + randomKey + '" frameborder="0" width="100%" height="100%"></iframe></div>';
    document.body.insertAdjacentHTML("beforeend", strCaptcha), window.removeEventListener("message", checkVerifyTildaCaptcha), window.addEventListener("message", checkVerifyTildaCaptcha), document.getElementById("tildaformcaptchabox").focus()
    ...
    if (!t_hasClass(body, "t-body_scroll-locked")) {
        ...
        t_addClass(body, "t-body_scroll-locked"), body.style.top = "-" + bodyScrollTop + "px", body.setAttribute("data-popup-scrolltop", bodyScrollTop)
    }
}, window.tildaForm.unlockBodyScroll = function () {
    var body = document.body;
    if (t_hasClass(body, "t-body_scroll-locked")) {
        var bodyScrollTop = body.getAttribute("data-popup-scrolltop");
        t_removeClass(body, "t-body_scroll-locked"), body.style.top = null, body.removeAttribute("data-popup-scrolltop"), document.documentElement.scrollTop = parseInt(bodyScrollTop)
    }
}, window.tildaForm.showSuccessPopup = function (message) {
    var strHtml = "", successPopup = document.getElementById("tildaformsuccesspopup"),
        successPopupText = document.getElementById("tildaformsuccesspopuptext"), body = document.body;
    if (!successPopup) {
    ...
    successPopupText.innerHTML = message, t_fadeIn(successPopup), t_addClass(body, "t-body_success-popup-showed"), /iPhone|iPad|iPod/i.test(navigator.userAgent) && !window.MSStream && setTimeout((function () {
    ...
    if (inputItsGood || form.insertAdjacentHTML("beforeend", '<div style="position: absolute; left: -5000px; bottom: 0; display: none;"><input type="text" name="form-spec-comments" value="Its good" class="js-form-spec-comments" tabindex="-1" /></div>'), 2 === formType || !formType && formKey) {
        ...
        if (dataForm = t_serializeArray(form), Array.prototype.forEach.call(dataForm, (function (data) {
        ...
        return xhr.open("POST", formUrl, !0), xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8"), xhr.setRequestHeader("Accept", "application/json, text/javascript, */*; q=0.01"), xhr.onreadystatechange = function () {
            if (4 === xhr.readyState) if (xhr.status >= 200 && xhr.status < 400) {
                    ...
                    if ("object" == typeof objData) {
                        ...
                        if (objData && objData.error) {
                            ...
                            Array.prototype.forEach.call(allError, (function (error) {
                                ...
                                var errorWrapperText = error.querySelector(".t-form__errorbox-text");
                                errorWrapperText ? errorWrapperText.innerHTML = objData.error : error.innerHTML = objData.error, error.style.display = "block"
                ...
                if (!xhr || 0 == xhr.status && tsDelta < 100) Array.prototype.forEach.call(allError, (function (error) {
                    ...
                    xhr && xhr.responseText ? Array.prototype.forEach.call(allError, (function (error) {
                        error.innerHTML = "[" + xhr.status + "] " + xhr.responseText + ". Please, try again later."
                    })) : xhr && xhr.statusText ? Array.prototype.forEach.call(allError, (function (error) {
        ...
        return xhr.open("POST", form.getAttribute("action"), !0), xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8"), xhr.setRequestHeader("Accept", "text/plain, */*; q=0.01"), xhr.onreadystatechange = function () {
            if (4 === xhr.readyState) if (xhr.status >= 200 && xhr.status < 400) {
                var successBox = form.querySelector(".js-successbox"),
                    ...
                    dataSuccessCallback = form.getAttribute("data-success-callback"), data;
                if (t_removeClass(btnSubmit, "t-btn_sending"), btnSubmit.tildaSendingStatus = "0", data = xhr.responseText) if ("{" == data.substring(0, 1)) {
                    var objData = JSON.parse(data);
                    if ("object" == typeof objData) if (objData && objData.message && "OK" !== objData.message) successBox.innerHTML = objData.message; else if (objData && objData.error) {
                        var msgContainers,
                            errorBoxes = (msgContainers = t_forms__getErrorContainers(form, "Unknown error. Please, try again later.")).errorBoxes,
                            allError = msgContainers.allError;
                        return Array.prototype.forEach.call(allError, (function (error) {
                ...
                } else successBox.innerHTML = data, t_parseScripts(successBox, "");
                var linkPage = "/tilda/" + formId + "/submitted/", title = "Send data from form " + formId;
                window.Tilda && "function" == typeof window.Tilda.sendEventToStatistics ? window.Tilda.sendEventToStatistics(linkPage, title, "", 0) : "undefined" != typeof ga && "tilda" !== window.mainTracker ? ga("send", {
                    hitType: "pageview",
                    page: linkPage,
                    title: title
                }) : window.mainMetrika > "" && window[window.mainMetrika] && window[window.mainMetrika].hit(linkPage, {
                    title: title,
                    referer: window.location.href
                }), t_triggerEvent(form, "tildaform:aftersuccess"), window.tildaForm.successEnd(form, dataSuccessUrl, dataSuccessCallback)
            } else {
                t_removeClass(btnSubmit, "t-btn_sending"), btnSubmit.tildaSendingStatus = "0";
                var msgContainers, errorBoxes = (msgContainers = t_forms__getErrorContainers(form, "")).errorBoxes,
                    allError = msgContainers.allError, data = xhr.responseText;
                Array.prototype.forEach.call(allError, (function (error) {
                    data ? error.innerHTML = data + ". Please, try again later. [" + xhr.status + "]" : xhr.statusText ? error.innerHTML = "Error [" + xhr.statusText + "]. Please, try again later. [" + xhr.status + "]" : error.innerHTML = "Unknown error. Please, try again later. [" + xhr.status + "]", error.style.display = "block"
                })), Array.prototype.forEach.call(errorBoxes, (function (error) {
                    error.style.display = "block"
                })), t_addClass(form, "js-send-form-error"), t_triggerEvent(form, "tildaform:aftererror")
            }
...
function t_forms__initForms() {
    ...
    window.t_forms__inputData = {}, t_forms__addRecaptcha(), Array.prototype.forEach.call(recBlocks, (function (rec) {
        var recid = rec.id;
        window.initForms[recid] || (t_forms__initEventPlaceholder(rec), t_forms__addInputItsGood(rec), t_forms__addAttrAction(rec), t_forms__onSubmit(rec), t_forms__onClick(rec), t_forms__onRender(rec), t_forms__addFocusOnTab(rec), t_onFuncLoad("t_throttle", (function () {
        ...
        if (focusableElements) {
            ...
            document.addEventListener("keydown", (function () {
                ...
                focusableElement.addEventListener("focus", (function () {
                    "keyboard" === handlerType && ((focusableElement.classList.contains("t-input_pvis") || focusableElement.classList.contains("t-input-phonemask")) && (focusableElement = focusableElement.parentElement), focusableElement.classList.add("t-focusable"), handlerType = null)
...
function t_forms__initEventPlaceholder(rec) {
    var eventFocus = "focus", eventBlur = "blur";
...
function t_forms__removePlaceholder(event) {
    ...
    if ("INPUT" === input.tagName) {
        var inputGroup = input.closest("[data-input-lid]"), strPlace = input.getAttribute("placeholder"), inputId = "";
        if (inputGroup) inputId = inputGroup.getAttribute("data-input-lid"); else {
...
function t_forms__moveToErrorInput(inputId, currentErrorItem) {
    ...
    currentErrorItem && (currentErrorItem.removeEventListener("click", t_forms__focusIntoInput), currentErrorItem.addEventListener("click", t_forms__focusIntoInput))
}

function t_forms__addPlaceholder(event) {
    var input = (event = event || window.event).target || event.srcElement,
        inputGroup = input.closest("[data-input-lid]"), inputId = "";
    if (inputGroup) inputId = inputGroup.getAttribute("data-input-lid"); else {
        var form = input.closest("form");
        form && (inputId = form.getAttribute("data-input-lid"))
    }
...
function t_forms__calculateInputsWidth(recId) {
    ...
    if (recId || !document.body.classList.contains("t706__body_cartwinshowed") && !document.body.classList.contains("t706__body_cartpageshowed") || (rec = document.querySelector(".t706")), rec) {
        var formWrapper = rec.querySelector(".t-form__inputsbox");
        if (formWrapper) {
            formWrapper.closest(".t706") && formWrapper.classList.add("t-form__inputsbox_inrow");
            var fields = rec.querySelectorAll(".t-input-group_widthdef, .t-input-group_inrow");
            if (formWrapper.classList.contains("t-form__inputsbox_inrow") || (fields = rec.querySelectorAll(".t-input-block_width")), 0 !== fields.length) {
                (formWrapper.classList.contains("t-form__inputsbox_vertical-form") || formWrapper.closest(".t706")) && formWrapper.classList.add("t-form__inputsbox_flex");
                var formWrapperWidth = formWrapper.offsetWidth, fieldsType = ["rd", "ri", "uc", "ws", "hd", "fr", "st"];
                Array.prototype.forEach.call(fields, (function (field) {
                    if (formWrapper.classList.contains("t-form__inputsbox_inrow") || (field = field.closest(".t-input-group")), field && field.classList.contains("t-input-group_inrow-withsibling")) {
                        var nextField = field.nextElementSibling, incorrectType = fieldsType.filter((function (type) {
                            return nextField.classList.contains("t-input-group_" + type)
                        }));
                        if (!nextField || 0 !== incorrectType.length || !(nextField.classList.contains("t-input-group_inrow") && !field.classList.contains("t-input-group_widthdef") || field.classList.contains("t-input-group_widthdef") && nextField.classList.contains("t-input-group") && !nextField.classList.contains("t-input-group_inrow"))) return field.classList.remove("t-input-group_inrow-withsibling"), void field.classList.add("t-input-group_inrow-last");
                        nextField.classList.add("t-input-group_inonerow")
                    } else field.classList.add("t-input-group_inrow-last");
                    nextField && field.classList.contains("t-input-group_widthdef") && field.classList.contains("t-input-group_inrow-withsibling") && nextField.classList.contains("t-input-group_inonerow") && !nextField.classList.contains("t-input-group_inrow") && (nextField.classList.add("t-input-group_widthdef"), nextField.classList.contains("t-input-group_inrow-withsibling") || nextField.classList.add("t-input-group_inrow-last"), nextField.classList.contains("t-input-group_inrow") && field.classList.add("t-input-group_inrow-last")), formWrapper.classList.contains("t-form__inputsbox_inrow") || t_forms__calculateFieldsWidthInJS(field, formWrapperWidth)
                }));
                var allLastFieldsWithCertainWidth = rec.querySelectorAll(".t-input-group_inrow.t-input-group_inonerow.t-input-group_inrow-last");
                allLastFieldsWithCertainWidth.length > 0 && t_forms__moveFieldToNextRow(allLastFieldsWithCertainWidth);
                var allLastFieldsWithDefWidth = rec.querySelectorAll(".t-input-group_widthdef.t-input-group_inrow-last");
                formWrapper.classList.contains("t-form__inputsbox_inrow") && allLastFieldsWithDefWidth.length > 0 && t_forms__combineFieldsWithDefWidth(allLastFieldsWithDefWidth, recId)
            }
        }
    }
}
...
function t_forms__moveFieldToNextRow(allLastFields) {
    var rows;
    t_forms__createArrWithAllRows(allLastFields, "t-input-group_inrow").forEach((function (row) {
        var isLongNextField = !1;
        if (row.forEach((function (field) {
            field.classList.contains("t-input-group_inrow-last") && field.nextElementSibling && !field.nextElementSibling.classList.contains("t-input-group_inonerow") && (isLongNextField = !0)
        })), !isLongNextField) {
            var commonWidth = row.reduce((function (sum, field) {
                var fieldClasses, widthClass = field.classList.value.split(" ").filter((function (fieldClass) {
                    return -1 !== fieldClass.indexOf("t-input-group_width")
                })), characterQuantity = 2, fieldWidth;
                return -1 !== (widthClass = widthClass[0]).indexOf("100") && (characterQuantity = 3), sum + Number(widthClass.substr(widthClass.length - 2, characterQuantity))
            }), 0);
            commonWidth <= 100 || row.forEach((function (field) {
                field.classList.contains("t-input-group_inrow-last") && (field.style.marginRight = "calc(100% - (" + (commonWidth - 100) + "%)", field.style.flex = "0 0 auto")
            }))
        }
    }))
}
...
function t_forms__combineFieldsWithDefWidth(allLastFields, recId) {
    var allRowsWithDefWidth = [], rows;
    t_forms__createArrWithAllRows(allLastFields, "t-input-group_widthdef").forEach((function (row) {
        var maxRowLength = 4;
        if (row.length > 4) for (var i = 0; i < row.length; i += 4) {
        ...
        var rowWidth, widthClass = "t-input-group_width" + ({4: "25", 3: "33", 2: "50"}[row.length] || "100");
        row.forEach((function (field) {
            if (field.classList.add(widthClass), field.classList.contains("t-input-group_rg")) {
...
function t_forms__createArrWithAllRows(allLastFields, certainClass) {
    var allSequences = [], oneSequence = [];
    return Array.prototype.forEach.call(allLastFields, (function (field) {
        for (oneSequence.push(field); field.previousElementSibling && field.previousElementSibling.classList.contains(certainClass) && !field.previousElementSibling.classList.contains("t-input-group_inrow-last");) field = field.previousElementSibling, oneSequence.push(field);
        allSequences.push(oneSequence.reverse()), oneSequence = []
    })), allSequences
}

function t_forms__calculateFieldsWidthInJS(field, formWrapperWidth) {
    var marginLeft = 15, columns = {
        1: "t-input-block_width100",
        2: "t-input-block_width50",
        3: "t-input-block_width33",
        4: "t-input-block_width25"
    };
    for (var i in columns) if (field.classList.contains(columns[i])) {
        field.style.width = (formWrapperWidth - 15 * (i - 1)) / i + "px";
        var label = field.previousElementSibling;
        label && label.classList.contains("t-input-title") && (label.style.width = (formWrapperWidth - 15 * (i - 1)) / i + "px"), window.innerWidth < 480 && (field && (field.style.width = "100%"), label && (label.style.width = "100%"))
    }
}
...
function t_forms__initBtnClick(event) {
    ...
    if (btnSubmit) {
        ...
        if (form) {
            ...
            var formId = form.getAttribute("id"), arrErrors = [], btnStatus = "";
            if (btnSubmit.tildaSendingStatus && (btnStatus = btnSubmit.tildaSendingStatus), !(btnStatus && btnStatus >= 1 || t_hasClass(btnSubmit, "t706__submit_disable"))) {
                if (t_addClass(btnSubmit, "t-btn_sending"), btnSubmit.tildaSendingStatus = "1", window.tildaForm.hideErrors(form), arrErrors = window.tildaForm.validate(form), window.tildaForm.showErrors(form, arrErrors)) return t_removeClass(btnSubmit, "t-btn_sending"), void (btnSubmit.tildaSendingStatus = "0");
                var allRecords, formKey = document.getElementById("allrecords").getAttribute("data-tilda-formskey"),
                    formType = parseInt(form.getAttribute("data-formactiontype")), inputsServices;
                if (!form.querySelectorAll(".js-formaction-services").length && 1 !== formType && !formKey) {
                    ...
                    })), t_addClass(form, "js-send-form-error"), t_removeClass(btnSubmit, "t-btn_sending"), btnSubmit.tildaSendingStatus = "0", void t_triggerEvent(form, "tildaform:aftererror")
                }
                if (form.querySelector(".g-recaptcha") && grecaptcha) {
                    ...
                    if (captchaId) grecaptcha.reset(captchaId); else {
                        var opts = {
                            size: "invisible",
                            sitekey: form.getAttribute("data-tilda-captchakey"),
                            callback: window.tildaForm.captchaCallback
                        };
                        captchaId = grecaptcha.render(formId + "recaptcha", opts), form.tildaCaptchaClientId = captchaId
                    }
...
function t_forms__submitEvent(event) {
    ...
    if (event.target && (form = event.target), form) {
        var btnSubmit = form.querySelector('[type="submit"]'), btnStatus = "";
        btnSubmit && btnSubmit.tildaSendingStatus && (btnStatus = btnSubmit.tildaSendingStatus), btnStatus && "3" === btnStatus ? btnSubmit.tildaSendingStatus = "" : (btnSubmit && !t_hasClass(btnSubmit, "t706__submit_disable") && btnSubmit.click(), event.preventDefault ? event.preventDefault() : event.returnValue = !1)
    }
}
...
function t_asyncLoad(input) {
    ...
    dataMask && !dataInit && (dataPlaceholder ? t_onFuncLoad("t_customMask__mask", (function () {
...
function t_forms__getErrorContainers(form, innerText) {
    ...
    return errorBoxes.length || (form.insertAdjacentHTML("afterbegin", '<div class="js-errorbox-all"></div>'), errorBoxes = form.querySelectorAll(".js-errorbox-all")), allError.length || (Array.prototype.forEach.call(errorBoxes, (function (error) {
    ...
    Array.prototype.forEach.call(allRecaptchas, (function (recaptcha) {
        ...
        if (captchaKey) {
            ...
            if (!window.tildaForm.isRecaptchaScriptInit) {
                ...
                window.tildaForm.isRecaptchaScriptInit = !0, script.type = "text/javascript", script.src = "https://www.google.com/recaptcha/api.js?render=explicit", script.async = !0, head.appendChild(script), head.insertAdjacentHTML("beforeend", '<style type="text/css">.js-send-form-success .grecaptcha-badge {display: none;}</style>')
            }
            document.getElementById(formId + "recaptcha") || recaptcha.insertAdjacentHTML("beforeend", '<div id="' + formId + 'recaptcha" class="g-recaptcha" data-sitekey="' + captchaKey + '" data-callback="window.tildaForm.captchaCallback" data-size="invisible"></div>')
        } else t_removeClass(recaptcha, "js-tilda-captcha")
    }))
}

function t_forms__getMsg(msg) {
    var dict = [], lang = window.t_forms__lang;
    return dict.EN = {
    ...
    }, "function" == typeof t_forms__getDict && "RU" !== lang && "EN" !== lang && (dict = t_forms__getDict()), dict[lang] ? dict[lang][msg] : dict.EN[msg]
}

function checkVerifyTildaCaptcha(event) {
    if (-1 !== (event = event || window.event).origin.indexOf(window.tildaForm.endpoint)) {
        var tildaCaptcha = document.getElementById("js-tildaspec-captcha"),
            formCaptchaBox = document.getElementById("tildaformcaptchabox");
        if ("closeiframe" == event.data) return formCaptchaBox && t_removeEl(formCaptchaBox), void (tildaCaptcha && t_removeEl(tildaCaptcha));
        var capthaKey = event.data;
        if (tildaCaptcha) {
            tildaCaptcha.value = capthaKey, formCaptchaBox && t_removeEl(formCaptchaBox);
...
function t_parseScripts(main, className) {
    ...
    Array.prototype.forEach.call(scripts, (function (oldScript) {
        for (var newScript = document.createElement("script"), j = 0; j < oldScript.attributes.length; j++) {
            var attr = oldScript.attributes[j];
            newScript.setAttribute(attr.name, attr.value)
        }
        if (oldScript.innerHTML.length) newScript.appendChild(document.createTextNode(oldScript.innerHTML)), oldScript.parentNode.replaceChild(newScript, oldScript); else {
            var script = document.createElement("script");
            script.src = oldScript.attributes.src.value, main.appendChild(script), t_removeEl(oldScript)
        }
    }))
}
...
function t_forms__onSuccess(form) {
    form instanceof Element || (form = form[0]);
    var rec = form.closest(".r"), recType = rec.getAttribute("data-record-type"),
        ...
        inputWrapperPaddingBottom = parseInt(inputWrapperStyle.paddingBottom) || 0, inputWrapperHeight, inputOffset,
        inputBottom = inputWrapper.clientHeight - (inputWrapperPaddingTop + inputWrapperPaddingBottom) + (inputWrapper.getBoundingClientRect().top + window.pageYOffset),
        successBox,
        successBoxOffset = form.querySelector(".t-form__successbox").getBoundingClientRect().top + window.pageYOffset,
        target = 0, windowHeight = window.innerHeight, body = document.body, html = document.documentElement,
        documentHeight = Math.max(body.scrollHeight, body.offsetHeight, html.clientHeight, html.scrollHeight, html.offsetHeight);
    if (121 == recType) {
        var successCallbackName = form.getAttribute("data-success-callback");
        successCallbackName && (recType = successCallbackName.split("_onSuccess")[0].replace("t", ""))
    }
    var inputsboxHiddenClassName = "t" + recType + "__inputsbox_hidden",
        arrNotScroll = [702, 708, 862, 945, 1014, 1114], isScroll = !0;
    target = window.innerWidth > 960 ? successBoxOffset - 200 : successBoxOffset - 100;
    var tildaLabel = document.querySelector(".t-tildalabel");
    if (successBoxOffset > window.scrollY || documentHeight - inputBottom < windowHeight - 100) inputWrapper.classList.add(inputsboxHiddenClassName), windowHeight > documentHeight && tildaLabel && setTimeout((function () {
        t_fadeOut(tildaLabel)
    }), 300); else {
        for (var i = 0; i < arrNotScroll.length; i++) if (arrNotScroll[i] == recType) {
            isScroll = !1;
            break
        }
        isScroll && t_forms__scrollBeginForm(target), setTimeout((function () {
            inputWrapper.classList.add(inputsboxHiddenClassName)
        }), 400)
    }
    var successUrl = form.getAttribute("data-success-url");
    if (successUrl && setTimeout((function () {
        window.location.href = successUrl
    }), 500), 835 == recType || 862 == recType) {
        var prevBtn = rec.querySelector(".t" + recType + "__btn_prev"),
            quizWrapper = rec.querySelector(".t" + recType + "__wrapper"),
            quizFormWrapper = rec.querySelector(".t" + recType + "__quiz-form-wrapper");
        prevBtn && (prevBtn.style.display = "none"), quizWrapper && (quizWrapper.style.minHeight = ""), quizFormWrapper && (quizFormWrapper.style.minHeight = "")
    }
}
...
function t_forms__scrollBeginForm(target) {
    var duration = 400,
        ...
        change = target - start, currentTime = 0, increment = 16;

    function t_forms__easeInOutCubic(currentTime) {
        return (currentTime /= 200) < 1 ? change / 2 * currentTime * currentTime * currentTime + start : change / 2 * ((currentTime -= 2) * currentTime * currentTime + 2) + start
    }

    function t_forms__animateScroll() {
        currentTime += 16, window.scrollTo(0, t_forms__easeInOutCubic(currentTime)), currentTime < 400 ? setTimeout(t_forms__animateScroll, 16) : document.body.removeAttribute("data-scrollable")
    }
...
Array.prototype.some || (Array.prototype.some = function (fn) {
    ...
    for (var t = Object(this), len = t.length >>> 0, thisArg = arguments.length >= 2 ? arguments[1] : void 0, i = 0; i < len; i++) if (i in t && fn.call(thisArg, t[i], i, t)) return !0;
    return !1
}), function (e) {
    ...
    var n = 0 | fromIndex;
    if (n >= len) return -1;
    for (k = Math.max(n >= 0 ? n : len - Math.abs(n), 0); k < len; k++) if (k in o && o[k] === searchElement) return k;
    return -1
});
...
function t_serializeArray(form) {
    for (var arr = [], elements = form.querySelectorAll("input, textarea, button, select"), i = 0; i < elements.length; i++) {
        ...
        if (!(!elements[i].name || elements[i].disabled || ["file", "reset", "submit", "button"].indexOf(elements[i].type) > -1 || inputGroup && inputGroup.getAttribute("data-hidden-by-condition"))) if ("select-multiple" !== elements[i].type) ["checkbox", "radio"].indexOf(elements[i].type) > -1 && !elements[i].checked || arr.push({
            name: elements[i].name,
            value: elements[i].value
        }); else for (var options = elements[i].options, j = 0; j < options.length; j++) options[j].selected && arr.push({
            name: options[j].name,
...
function t_fadeOut(el) {
    if ("none" !== el.style.display) var opacity = 1, timer = setInterval((function () {
        el.style.opacity = opacity, (opacity -= .1) <= .1 && (clearInterval(timer), el.style.display = "none", el.style.opacity = null)
    }), 30)
}
...
function t_fadeIn(el) {
    if ("block" !== el.style.display) {
        ...
        var timer = setInterval((function () {
            el.style.opacity = opacity, (opacity += .1) >= 1 && clearInterval(timer)
        }), 30)
    }
}
